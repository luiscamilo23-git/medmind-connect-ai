...
const corsHeaders = {
  ...
  'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type, x-siigo-signature, x-supabase-client-platform, x-supabase-client-platform-version, x-supabase-client-runtime, x-supabase-client-runtime-version',
};
...
Deno.serve(async (req) => {
  ...
  try {
    console.log('Siigo webhook received');
    ...
    console.log('Siigo payload:', JSON.stringify(payload, null, 2));

    // Validate webhook signature (Siigo specific)
    const signature = req.headers.get('x-siigo-signature');
    if (signature) {
      console.log('Siigo signature:', signature);
    }

    // Extract event data
    const eventType = payload.event || payload.type || 'unknown';
    const invoiceReference = payload.document?.id || payload.id || payload.name;
    ...
    if (eventType.includes('approved') || eventType.includes('authorized') || payload.status === 'approved') {
      ...
      if (payload.cufe) {
        invoiceUpdates.cufe = payload.cufe;
      }
      if (payload.number || payload.name) {
        invoiceUpdates.numero_factura_dian = payload.number || payload.name;
      }
